...
namespace RBIPL {
    ...
    {
        Segment seg;
        for(unsigned int i = 0; i < ref_tree.getNrOfSegments(); i++ ) {
    ...
    {
        if( tree_param.rows() != 10*ref_tree.getNrOfSegments() ) updateParams();
        return tree_param;
    }
    ...
    bool TreeInertialParameters::changeInertialParameters(const Eigen::VectorXd & new_chain_param,  Tree& new_tree)
    {
        ...
        for(unsigned int i = 0; i < ref_tree.getNrOfSegments(); i++ ) {
            seg = seg_vector[i]->second.segment;
            seg.setInertia(deVectorize(tree_param.segment(i*10,10)));
    ...
    TreeInertialParameters::TreeInertialParameters(Chain & chain) : ref_chain(chain), 
